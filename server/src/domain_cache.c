...
static int name_append(struct rbtree *name_tree,
                       char *         domain,
                       int            d_length,
    ...
    char *         pos;
    struct rbnode *node;
    DOMAIN_CACHE * cache = (DOMAIN_CACHE *)calloc(
            1, sizeof(DOMAIN_CACHE) + d_length + 1 + an_length);
    if (cache) {
...
static int wname_append(struct rbtree *name_tree,
                        char *         prefix,
                        int            p_length,
                        char *         domain,
                        int            d_length,
    ...
    struct rbnode *node;
    DOMAIN_CACHE * cache = (DOMAIN_CACHE *)calloc(
            1, sizeof(DOMAIN_CACHE) + p_length + d_length + 2 + an_length);
    if (cache) {
        pos             = cache->buffer;
        cache->prefix   = pos;
        cache->p_length = p_length;
        if (p_length > 0) memcpy(cache->prefix, prefix, p_length);
        pos += p_length + 1;
        cache->domain   = pos;
        cache->d_length = d_length;
        if (d_length > 0) memcpy(cache->domain, domain, d_length);
        pos += d_length + 1;
        cache->answer    = pos;
        cache->an_count  = 1;
        cache->an_length = an_length;
...
void domain_cache_init(const char *hosts_file) {
    ...
    if (fp) {
        while (fgets(line, sizeof(line) - 1, fp)) {
            ...
            ip = skip_space(line);
            if (is_space(*ip) || *ip == '#') continue;
            rear = skip_to_space(ip);
            if (!is_space(*rear)) continue;
            *rear       = '\0';
            ...
            *(unsigned short *)pos = htons(sizeof(addr));
            ...
            while (rear + 1 < rlimit) {
                ...
                if (is_space(*domain)) break;
                rear = skip_to_space(domain);
                if (*rear && is_space(*rear)) *rear = '\0';
                ...
                if (wildcard) {
                    *wildcard++ = '\0';
                    ...
                    if (name_append(&g_cache.rb_name,
                                    domain,
                                    d_length,
                                    an_length,
                                    answer))
    ...
    while ((node = rbtree_last(&rb_wname)) != RBNODE_NULL) {
        ...
        if (!list_empty(&g_cache.lh_name)) {
            ...
            if (cache1->d_length <= cache->d_length
                && strncmp(cache1->domain,
                           cache->domain
                                   + (cache->d_length - cache1->d_length),
                           cache1->d_length)
                           == 0)
                cache1->d_same = 1;
        }
...
DOMAIN_CACHE *domain_cache_search(char *domain) {
    ...
    if (node == RBNODE_NULL) {
        if (g_cache.wcount < 1) return NULL;
        same   = 0;
        cache1 = NULL;
        list_for_each(cache, &g_cache.lh_name, DOMAIN_CACHE, lh_name) {
            pos = ctx.length - cache->d_length;
            if (same) same = cache->d_same;
            if (same
                || (pos >= 0
                    && strcmp(ctx.domain + pos, cache->domain) == 0)) {
                same = 1;
                if (cache->p_length == 0) {
                    cache1 = cache;
                    break;
                ...
                if (pos1 >= 0
                    && strncmp(ctx.domain, cache->prefix, cache->p_length)
                               == 0) {
                    cache1 = cache;
                    break;
                }
