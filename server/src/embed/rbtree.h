...
#define RBNODE_EMPTY NULL

#define rbtree_entry(ptr, type, member) container_of(ptr, type, member)

#define rbnode_entry(ptr, type, member) container_of(ptr, type, member)
...
static inline struct rbnode *rbtree_insert_broken(struct rbtree *rbtree,
                                                  struct rbnode *data) {
    return rbtree_insert3(rbtree, data, 1);
}

static inline struct rbnode *rbtree_insert_replace(struct rbtree *rbtree,
                                                   struct rbnode *data) {
