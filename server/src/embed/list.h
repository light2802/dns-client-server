...
static inline void list_insert_list(struct list_head *list,
                                    struct list_head *other) {
    if (list_empty(other)) return;
...
#define list_first(ptr, type, member) container_of((ptr)->next, type, member)
...
#define list_for_each_reverse_safe(pos, tmp, head, type, member) \
    for (pos = 0,                                                \
        ...
        tmp  = container_of((pos)->member.prev, type, member);   \
         &pos->member != (head);                                 \
         pos = tmp, tmp = container_of(pos->member.prev, type, member))
