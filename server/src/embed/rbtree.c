...
struct rbnode rbnode_null = {&rbnode_null,
                             &rbnode_null,
                             &rbnode_null,
                             RBCOLOR_BLACK};
...
static void rbtree_rotate_left(struct rbtree *rbtree, struct rbnode *node) {
    ...
    if (right->left != RBNODE_NULL) right->left->parent = node;
...
static void rbtree_rotate_right(struct rbtree *rbtree, struct rbnode *node) {
    ...
    if (left->right != RBNODE_NULL) left->right->parent = node;
...
static inline void change_parent_ptr(struct rbtree *rbtree,
...
struct rbnode *
rbtree_insert3(struct rbtree *rbtree, struct rbnode *data, int flag) {
    int            r      = 0;
    ...
    while (node != RBNODE_NULL) {
        ...
        if (r < 0)
            ...
            if (node->left != RBNODE_NULL) node->left->parent = data;
            if (node->right != RBNODE_NULL) node->right->parent = data;
...
static inline void change_child_ptr(struct rbnode *child,
                                    struct rbnode *old,
                                    struct rbnode *new) {
    if (child == RBNODE_NULL) return;
    if (child->parent == old) child->parent = new;
}

static void rbtree_delete_balance(struct rbtree *rbtree,
                                  struct rbnode *child,
    ...
    while (go_up) {
        if (child_parent == RBNODE_NULL) return;
        ...
        if (child_parent->color == RBCOLOR_BLACK
            ...
            if (sibling != RBNODE_NULL) sibling->color = RBCOLOR_RED;
            child        = child_parent;
    ...
    if (child_parent->color == RBCOLOR_RED && sibling->color == RBCOLOR_BLACK
        && sibling->left->color == RBCOLOR_BLACK
        && sibling->right->color == RBCOLOR_BLACK) {
        if (sibling != RBNODE_NULL) sibling->color = RBCOLOR_RED;
    ...
    if (child_parent->right == child && sibling->color == RBCOLOR_BLACK
        ...
        if (child_parent->right == child)
            ...
            sibling = child_parent->right;
    } else if (child_parent->left == child && sibling->color == RBCOLOR_BLACK
...
void rbtree_delete(struct rbtree *rbtree, struct rbnode *to_delete) {
    ...
    if (rbnode_empty(to_delete)) return;

    if (to_delete->left != RBNODE_NULL && to_delete->right != RBNODE_NULL) {
    ...
    if (to_delete->color == RBCOLOR_RED)
        ...
        if (child != RBNODE_NULL) child->color = RBCOLOR_BLACK;
    } else
...
struct rbnode *rbtree_first(struct rbtree *rbtree) {
    ...
    if (rbtree->root != RBNODE_NULL) {
        for (node = rbtree->root; node->left != RBNODE_NULL; node = node->left)
            ;
    }
    return node;
}
...
struct rbnode *rbtree_last(struct rbtree *rbtree) {
    ...
    if (rbtree->root != RBNODE_NULL) {
        for (node = rbtree->root; node->right != RBNODE_NULL;
             node = node->right)
            ;
    }
    return node;
...
struct rbnode *rbtree_next(struct rbnode *node) {
    ...
    if (node->right != RBNODE_NULL) {
        for (node = node->right; node->left != RBNODE_NULL; node = node->left)
            ;
    } else {
        parent = node->parent;
...
struct rbnode *rbtree_previous(struct rbnode *node) {
    ...
    if (node->left != RBNODE_NULL) {
        for (node = node->left; node->right != RBNODE_NULL; node = node->right)
            ;
    } else {
        parent = node->parent;
