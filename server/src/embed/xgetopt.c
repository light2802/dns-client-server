...
int xgetopt(int                   argc,
    ...
    if (argc < 1 || argv == NULL || options == NULL || optind == NULL
        || optarg == NULL)
        return -1;
    ...
    *optarg = NULL;
    ...
    if (*cur_argv == '-') {
        if (*(++cur_argv) != '-') {
            if (*(cur_argv + 1) != '\0') embed_arg = cur_argv + 1;
            for (i = 0; options[i].opt != 0 || options[i].name; ++i) {
                if (options[i].opt == 0) continue;
            ...
            for (i = 0; options[i].opt != 0 || options[i].name; ++i) {
                if (strncmp(cur_argv, options[i].name, name_len)) continue;
